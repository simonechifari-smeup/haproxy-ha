#!/usr/bin/env python3
"""
validate-nodo1.py — Validazione configurazione nodo 1 (MASTER)
Adattare le variabili nella sezione "Valori attesi" all'ambiente reale.
Eseguire come root sul server: sudo python3 validate-nodo1.py
"""
...
NODE1_IP     = "<NODE1_MGMT_IP>"
NODE2_IP     = "<NODE2_MGMT_IP>"
VIP          = "<VIP_IP>"
GATEWAY      = "<GATEWAY_IP>"
BACKEND_IP   = "<NODE1_BACKEND_IP>"
...
try:
    ...
    if VIP in r.stdout: ok(f"VIP {VIP} presente (MASTER attivo)")
    else: warn(f"VIP {VIP} non presente — keepalived non avviato o nodo BACKUP")
